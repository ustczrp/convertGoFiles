...
def execute_prompt(prompt, taskName, client):
    ...
    print(f'Start executing prompt ({taskName}) at: {datetime.now().strftime("%m/%d/%Y, %H:%M:%S")} ...')
    ...
    end_time = time.time()
    print(f'Duration of chat_with_gpt4 for prompt ({taskName}): {end_time - start_time:.4f} seconds')
    ...
    if response_message.tool_calls:
        if response_message.tool_calls[0]:
            try:
                ...
                function_args = json.loads(tool_call.function.arguments, strict=False) # strict=False allows for parsing java code in the arguments
                print(f"Function call: {function_name}")  
                print(f"Function arguments: {function_args}")  
                
                if function_name == "extract_source_file":
                    function_response = extract_source_file(request=function_args.get("request"), source=function_args.get("source"), tool_call=tool_call)
                else:
